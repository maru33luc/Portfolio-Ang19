...
export class AboutComponent implements AfterViewInit, OnDestroy {
  ...
  /**
   * Configura IntersectionObserver para animar los items del timeline
   * al entrar en el viewport. Es síncrono, nativo y compatible con SSR.
   */
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            // Una vez revelado, dejar de observar este elemento
            this.observer?.unobserve(entry.target);
          }
        });
      },
      {
        threshold: 0.1,    // Revela cuando el 10% del elemento es visible
        rootMargin: '0px', // Sin margen adicional
      }
    );

    // Observar todos los items actuales del DOM
    const items = document.querySelectorAll('.timeline-item');
    items.forEach((el) => {
      // Resetear estado visual para que la animación vuelva a correr
  ...
  filterItems(type: 'all' | 'experience' | 'education') {
    ...
    // Forzar actualización sincrónica del DOM y luego configurar observer
